...
```{r}
str_length(fruit) #length of each entry
length(fruit) #looks at fruit as a vector (80 entries in the vector ie 80 fruits)
```
...
```{r}
str_view_all_match(countries, "^[^aeiouAEIOU]*$") #*=0 or more non-vowels till the end of the string ie repear a non-vowel as many times as necessary till the end of the word
```
...
```{r}
str_view_all(c("abad", "abbd"), pattern="(a)(b)\\1") #whatever was resolved in group 1 - a, then b and then group 1 which is a
...
```{r}
str_view_all(c("bananas", "Who can? Bananas can."), "(.)(.)\\1\\2.*\\1\\2") #first group reolves to a, 2nd to n b/c that is the one that works
```
