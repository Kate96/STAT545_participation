...
```
```{r}
pmap(list(a, b, c), ~ ..1 * ..2 * ..3) #..# allows to have any number of var specified
```
```{r}
...
map2_chr(name, commute, function(x, y) str_c(x, " takes ", y, " minutes to get to work."))
...
```{r}
map2(name, commute, function(s, t){
  str_c(s, " takes ", t, " minutes to get to work.") 
  })
```
...
Data frame and tibbles are lists.
...
got_alias <- got_chars %>% {
  tibble(name = map_chr(., "name"),
         aliases = map(., "aliases")) # . means mapping entire got_chars
  } #curly braces will not put got_chars as a 1st parameter of tibble
got_alias
```
