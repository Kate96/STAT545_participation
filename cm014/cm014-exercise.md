...
``` r
predict(my_lm) %>% 
  head() # takes x values in each row of gapminder ie log(gdppercap) and predicts y-value ie life expectancy
```
...
``` r
predict(my_lm, newdata = tibble(gdpPercap = c(500, 600, 700, 10000)))
```

    ##        1        2        3        4 
    ## 43.13342 44.66585 45.96150 68.31280
...
``` r
tidy(my_lm) # p-value of slope (loggdp) is close to 0 => sig linear effect on lifeExp
```
...
``` r
augment(my_lm) # resid = difference between fitted and actual values
```
...
``` r
glance(my_lm) #model is significant b/c p-value is low
```
